cc.Class({
     ...
     * @param times             计时器执行次数
     */
    runtimer:function(source,timernode, timer_first, times){

        let self = this ;
        this.remaining = times ;
        timer_first.string = times +"s";
        
        this.timersrc = function() {
            self.remaining = self.remaining - 1;
            if(self.remaining < 0){
                source.unschedule(this);
                timer_first.string = "获取" ;
               timernode.getComponent(cc.Button).interactable = 1;
            }else{
                timer_first.string = self.remaining +"s" ;
            }
        } ;
     ...
     runtimers:function(source , timernode  , atlas, timer_first , timer_sec , times){
